...
| ظ | th (Standardfall, 28 von 32 Bestandseinträgen) | tholl, thabt | dh NICHT pauschal als Fehler behandeln — echte lexikalisierte Ausnahmen bei häufigen Wörtern, Ninja-bestätigt: `dhhar`("Rücken", NICHT thahr — id 722, 2026-08-07 fälschlich auf thahr korrigiert und wieder zurückgesetzt), `la7dha`("Moment"). Vor jeder Korrektur eines ظ/ذ-Worts das konkrete Wort bei Ninja gegenchecken, nicht die Mehrheitsregel blind anwenden. Deshalb bewusst KEIN `TRANSLIT_RULES`-Check im Trainer dafür (anders als bei ح/خ/ع/غ/ق/ش/ض/ج/ز/ه/س, die praktisch ausnahmslos sind) — würde echte Bestandseinträge fälschlich flaggen. |
...
**Zweiter Teil, korrigiert:** Direkt danach wurde als vermeintlicher Bestandsfehler id 722 (`ظَهْر`="Rücken", `dhhar`) auf `thahr` "korrigiert" — das war ein Fehler. Nils fragte nach, warum der Trainer-Check (`TRANSLIT_RULES`) den Eintrag nie gemeldet hatte; die Antwort führte zur Erkenntnis, dass es für ظ/ذ (anders als für die anderen Konsonanten) **keinen** `TRANSLIT_RULES`-Check gibt — zu Recht, denn `dhhar` und `la7dha` sind Ninja-bestätigte lexikalisierte `dh`-Ausnahmen (siehe Pflichtregeln-Tabelle oben), keine Fehler. id 722 wieder auf `dhhar` zurückgesetzt. **Lehre:** Bevor ein per Mehrheitsregel gefundener "Ausreißer" korrigiert wird, IMMER das konkrete Wort einzeln bei Ninja gegenchecken — die Mehrheit ist eine Heuristik für den Default-Vorschlag, kein Beweis für den Einzelfall. Lohnt sich trotzdem als wiederkehrende Praxis: nach jeder neuen App-Funktion, die eine eigene Transliterationstabelle mitbringt, einmal Zeile für Zeile gegen die Pflichtregeln-Tabelle hier gegenlesen — aber Funde einzeln verifizieren, nicht pauschal durchziehen.
