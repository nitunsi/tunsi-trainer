...
- **User-Anfrage "Top 10 wichtigste unverlinkte Verben" (nach Korpus-Frequenz sortiert, `NOT EXISTS`-Check statt Bucket-Scan — deckt auch die Fälle ab, die der Bucket-Scan strukturell nie sieht):** `wsil`/ankommen(45), `tsawwir`/sich vorstellen(39), `shal`/fragen(35, TUNICO-Schreibweise für `sa'al` — nicht `s'al`, daher vorher übersehen), `zad`/zunehmen(30), `rawwa7`/nach Hause gehen(28), `3jib`/gefallen(26), `qass`/schneiden(26) gefunden und verlinkt (**124/300**). `khzar`(betrachten,32) und `thrab`(49, Bedeutung unklar) haben — per Substring-Suche auf `darija`/`arabic_script` gegengecheckt, nicht nur Skelett — **keine** echten Bestandstreffer (nur Nomen-Homographe: `khzana`=Schrank, `akthar`=mehr, `thra3`=Arm) — ohne jeden Anker nicht neu konstruiert.
- **`3ayyish`(33, freq52) doch im Bestand gefunden, User hatte recht:** Der Skelett-Scan hatte den Cluster verpasst, weil die Bestandszeilen fast alle mehrwortige Dankes-/Bitte-Formeln sind (`3ayshik`="danke", `y3ayshek`="Bitte" — wörtlich "möge [Gott] dich leben lassen", Kausativ-Ableitung von `3ash`/leben). Erklärt auch die hohe Korpus-Frequenz (idiomatischer Gebrauch, nicht wörtliches "ernähren"). Die zwei sauberen Kernformen (`3ayshik` id851, `y3ayshek` id1392) verlinkt, Konjugationstabelle mit der wörtlichen Kausativ-Bedeutung ("ernähren") als Referenz gebaut — **keine** Rotationszeile ergänzt, weil die übrigen Personen (`wir ernähren` etc.) hier keinen praktischen Lernwert haben (das Wort wird faktisch nur als feste Höflichkeitsformel gebraucht, nicht durchkonjugiert). Die übrigen ~15 Varianten-Zeilen sind mehrwortige Kombi-Phrasen (`"y3ayshek, el 3aqba lik"` etc.) — bewusst nicht einzeln verlinkt, gleiche Begründung wie bei anderen Phrasen-Einträgen oben.
- **User-Anfrage "Top 20" (nächste 20 nach Frequenz, gleiche Methode):** `t3arik`/sich streiten(22), `3awid`/wiederholen(21), `3ayyit`/rufen(20, per Substring statt Skelett gefunden — Bestandszeilen sind Phrasen wie "Ruf die Feuerwehr" und matchten daher nicht per Skelett-Array), `ghanna`/singen(20), `sawwir`/fotografieren(19), `lawwij`/suchen(16), `tsharrif`/sich geehrt fühlen(18) — gefunden und verlinkt (**132/300**). `sraq`/stehlen(24), `hamm`(23, unklare Bedeutung), `t3arrif`/kennenlernen(21), `tbaddil`/sich ändern(20), `qallaq`/beunruhigen(19), `kharrij`/hinausführen(19), `sajjil`/registrieren(18), `tabba3`/verfolgen(17), `dakhkhil`/hineinbringen(17), `sraf`/Geld wechseln(16), `thhur`/erscheinen(16) — alle per Skelett UND Substring-Suche gegengecheckt, **0 Bestandstreffer**, echte Korpuslücken.
- **User-Korrektur "ysarraf gibt es" + Aufforderung, die ganze Top-20-Liste nochmal gründlich zu prüfen (Substring-Suche mit weiteren Formmustern, u.a. Form-II-Geminationsvarianten wie `sarraf` statt nur `sraf`):**
  - `ysarraf`(id1095,"er wechselt €") **existiert wirklich**, gehört aber NICHT zu `tcv94`("sraf", Form I "Geld ausgeben") — `ysarraf` ist Form II ("Geld wechseln", andere Bedeutung), und dafür gibt es **keinen eigenen Korpus-Eintrag** unter den 300 (geprüft: kein `lemma_chatalpha`/`forms_chatalpha` mit "sarraf" in `tunico_corpus_verbs`). `tcv94` selbst hat also weiterhin 0 Bestandstreffer für seine eigenen attestierten Formen (`nusruf`,`sarft`,`yusruf`) — das war korrekt, nur die deutsche Kurz-Glosse "Geld wechseln" in der User-Antwort war ungenau (richtig: "Geld ausgeben").
  - **`thhur`(90, erscheinen) war doch im Bestand** — als Form-II-Kausativ `thahhher`/`ythahhher`("er zeigt/weist auf", id3415/3452), nicht als Form-I-Basis. Mit breiterem Substring-Muster (`thahh` statt `thhr`) gefunden, verlinkt + Rotationszeile ergänzt. **Stand: 133/300.**
  - `sraq`, `hamm`, `t3arrif`, `tbaddil`, `qallaq`, `kharrij`, `sajjil`, `dakhkhil`, `tabba3`, `khzar`, `thrab` **erneut mit breiteren Mustern gegengecheckt** (Form-II-Gemination, alternative Vokale, arabic_script-Suche ohne Diakritika-Annahme) — bleiben bestätigt bei 0 Treffern.
  - **Lehre: Form-II/-Kausativ-Varianten (geminierter Mittelkonsonant, z.B. `sraf`→`sarraf`, `thhur`→`thahhher`) gehören von Anfang an ins Substring-Suchmuster, nicht nur die Form-I-Grundform.** Wiederkehrender Fehlerpunkt in dieser Session.
- **User-Anfrage "Top 30" + direkt aus der Vokabelliste kopierte Bestandszeilen für 5 Verben (User hatte sie selbst im Trainer gefunden, exakte Schreibweise geliefert):** `b3id`/weggehen(15, +Vergangenheit+Rotation ergänzt), `kra`/mieten(14, +Vergangenheit+Rotation ergänzt), `jbid`/ziehen(13, 4 Zeilen bereits vollständig, nur verlinkt — inkl. der beiden Zeilen `yjbed`/`ejbed`, die in einem früheren Tranchen-Durchgang fälschlich als Fehltreffer für `bda` ausgeschlossen worden waren), `7awil`/versuchen(11, +Rotation ergänzt), `tayyish`/wegwerfen(11, +Präsens+Rotation ergänzt). **Stand: 138/300.**

### Root-Skelett-Suche (seit 2026-09-06, Abend) — verlässlichere Methode als Lemma-Raten

User-Kritik "kannst du die nicht selber finden?" nach wiederholten übersehenen Treffern (`shal`≠`s'al`, `sarraf`≠`sraf`, `thahhher`≠`thhur`) führte zu einer systematischeren Methode statt einzeln geratener Lemma-Schreibweisen:

```sql
CREATE OR REPLACE FUNCTION pg_temp.root_skel(t text) RETURNS text AS $$
  SELECT regexp_replace(
           regexp_replace(lower(coalesce(t,'')), '[aeiou''\s\-]', '', 'g'),
           '(.)\1+', '\1', 'g'
         );
$$ LANGUAGE sql IMMUTABLE;
```
Entfernt Vokale UND dedupliziert direkt aufeinanderfolgende gleiche Konsonanten — dadurch matchen `sarraf`/`sraf` (beide → `srf`) und `thahhher`/`thhur` (beide → `thr`) automatisch, ohne dass man die Gemination-Variante von Hand erraten muss. Angewendet: für jeden noch unverlinkten `tcv_id` `root_skel(lemma_chatalpha)` UND `root_skel()` jeder einzelnen `forms_chatalpha[]`-Form gegen `root_skel(darija)` **aller** noch unverlinkten `vocabulary`-Zeilen (nicht nur bestimmte Topics) matchen, ab Länge ≥3. Liefert deutlich mehr Kandidaten als der alte Skelett-Exact-Match — dafür **zwingend jeden Treffer manuell semantisch gegenchecken** (viele Fehltreffer bei kurzen Wurzeln, z.B. Nomen-Familien wie „weit/entfernt" für `b3id`, oder „Zucker" für `sakkar`).

**Bei diesem Durchgang (Top-30-Fortsetzung) gefunden und verlinkt:** `skit`/schweigen(14), `wja3`/schmerzen(11), `ban`/erscheinen(7), `bka`/weinen(6), `madd`/reichen(6), `tayya7`/runtergehen(5), `bas`/küssen(3), `ba77ar`/ans-Meer-gehen(3, über die abweichende Schreibvariante `ba7har`), `wakkil`/füttern(2), `brid`/kalt-werden(2 vorhandene Zeilen `bred`/`yibred` verlinkt), `qsad`/meinen-beabsichtigen(2), `qarra`/lehren(2, kombinierter Eintrag „qarra / 3allem"), `fsid`/verderben(2), `tfakkir`/sich-erinnern(2), `rba7`/gewinnen(2), `r7am`/Beileid-sprechen(2). **Stand: 154/300.**

**Kritischer Selbstfehler dabei, vom eigenen Duplikat-Check aufgefangen:** beim Bauen neuer Basis-/Rotationszeilen **4 echte Duplikate mit bereits bestehenden Zeilen erzeugt**, weil vor dem Schreiben nicht geprüft wurde, ob die geplante Zeile schon existiert:
- `bka`("er weinte") — existierende Zeile 552 hatte exakt denselben Inhalt, meine neue Zeile war eine reine Dublette. Gelöscht, 552 stattdessen in die Gruppe verlinkt.
- `ywakkil`("er füttert") — existierende Zeile 4233, identisch. Gelöscht, 4233 verlinkt.
- `yba7har`("er schwimmt im Meer / geht ans Meer") — existierende Zeile 4207, inhaltsgleich. Gelöscht, 4207 verlinkt.
- `madd`("er reichte", Vergangenheit) — existierende Zeile 3607 hatte identisches `darija`/`arabic_script`, aber als Imperativ ("gib her!") getaggt — im Tunesischen sind bei diesem Verbtyp Imperativ-Singular und 3.-Pers.-Vergangenheit lautgleich. Meine neue Zeile gelöscht; 3607 bleibt als Imperativ-Vertreter der Gruppe, die Vergangenheit wird nur noch über die Rotationszeile (1. Pers. Sg.) abgedeckt.
- Zusätzlich: `bint`("ich erschien", geplant als Rotationszeile für `ban`) kollidierte **wortgleich** mit dem sehr gebräuchlichen Nomen `bint`("Tochter", id3650) — grammatikalisch korrekt (Hohlwurzel-Verb, 1. Sg. Vergangenheit), aber pädagogisch verwirrend. Statt `homonym_ok` zu setzen, den Rotations-Inhalt auf `nban`("ich erscheine", Präsens) umgestellt, um die Kollision mit dem Alltagswort ganz zu vermeiden.
- **Lehre: vor jedem `INSERT` einer neuen Basis-/Rotationszeile erst `SELECT` auf exakte `darija`-Übereinstimmung im gesamten Bestand laufen lassen** — der nachgelagerte Duplikat-Check fängt es zwar ab, aber ein Vorab-Check spart die Korrekturrunde. Bei Homograph-Kollisionen mit sehr gebräuchlichen Alltagswörtern (nicht nur mit anderen Verben) lieber die Zell-Auswahl für die Rotationszeile wechseln als `homonym_ok` zu setzen, wenn eine kollisionsfreie Alternativform (andere Person/Zeit) verfügbar ist.

**Nachtrag: User fand per App-Duplikat-Check einen von mir übersehenen Fall — `kra`("er mietete", neue Vergangenheit-Grundform id4557) kollidierte mit der bestehenden Nomen-Zeile `kra`("Miete", id718).** Echter Homograph (Verb-Grundform = Nomen, unvermeidbar da `kra` die einzig korrekte 3.-Pers.-Vergangenheitsform ist), `homonym_ok=true` auf beiden gesetzt. Zeigt: der Vorab-`SELECT`-Check vor jedem `INSERT` (s.o.) muss auch gegen **komplett unabhängige Wortarten** (Nomen, nicht nur andere Verben) laufen, nicht nur gegen andere Verb-Kandidaten derselben Suche.

**Nachtrag: `3arref`/`y3arref`("vorstellen", id3381/3418) von der User gemeldet, war übersehen.** Grund: kein eigener Korpus-Eintrag für die Form-II-Aktivform "vorstellen" — nur `t3arrif`(tcv70, freq21, Form V "sich kennenlernen") existiert im Korpus, und `root_skel("t3arrif")`→`t3rf` vs. `root_skel("3arref")`→`3rf` matchen NICHT, weil das `t`-Präfix der Form V als echter Konsonant gewertet wird (kein Fehler der Funktion, sondern eine echte Grenze: verschiedene Verbformen/Measures desselben Verbs unterscheiden sich strukturell, nicht nur durch Vokale/Gemination). Trotzdem an `tcv70` verlinkt (nächstliegender Korpus-Anker, gleiche Wurzel ع-ر-ف, Form II "vorstellen" ist die aktivische Entsprechung der Form-V-Bedeutung) + Rotationszeile ergänzt. **Stand: 157/300.**
**Lehre: verschiedene Measures/Formen derselben Wurzel (Form I/II/III/V/VI) können lautlich so weit auseinanderliegen, dass auch `root_skel()` sie nicht mehr matcht — das ist eine strukturelle Grenze der Methode, kein behebbarer Suchfehler.** Nach dem "vollständigen Durchgang" oben sind daher wahrscheinlich noch vereinzelte Fälle dieser Art übrig; der Durchgang war vollständig für **eine** Measure pro Korpus-Lemma, nicht zwingend für alle abgeleiteten Formen.

**Vollständiger Durchgang (User-Auftrag "alle tunico Verben mal durchgehen"): `root_skel()` auf ALLE verbleibenden ~146 unverlinkten `tcv_id`s gegen den GESAMTEN unverlinkten Bestand angewendet (nicht nur Top-N nach Frequenz).** Ergebnis: `shamm`/riechen(10) und `t3ab`/müde werden(2, bestehende Zeile 1938 direkt als Rotationszeile weiterverwendet statt neu zu bauen) gefunden und verlinkt. **Endstand dieses Durchgangs: 156/300.** Alle übrigen ~144 `tcv_id`s liefern beim `root_skel`-Abgleich ausschließlich Fehltreffer (Nomen-/Adjektiv-Familien wie "nah/fast/näher" für `qrub`, "dick/fett" für `smin`, "lang/Tisch" für `twal`, Lehnwort-Speisen wie "Shakshouka"/"Brik", oder Partizipien wie "verboten"/"schwindelig" ohne finite Verbform) — **kein `tcv_id` mit echtem, aber übersehenem Bestandstreffer bleibt mehr übrig.** Diese Verben existieren im aktuellen Vokabelbestand schlicht nicht; sie kämen erst mit künftigen Imports dazu. Damit ist der TUNICO-Korpus-Verlinkungs-Durchgang für den aktuellen Bestand vollständig ausgeschöpft.
...
### Peace-Corps-Englisch-Gloss-Durchgang (seit 2026-09-06, User-Auftrag "schau mal ob du die restlichen bei peace corp findest")

Genau der oben als "künftiger Durchgang" beschriebene Ansatz: pro vermuteter englischer Glosse (`PREPARE`, `WASH`, `TIE`, `SWEEP`, `JUMP`, `FRY`, `SHAVE`, `PUSH`, `TASTE`, `JOKE`, `SHOP`, `HEAL`, `TRAIN`, `FREEZE`, `PROMISE`, `ORGANIZE`, `CLEAN`, `Stand up`, `PRINT`, `ACHIEVE`, `Turn off` u.a.) `peacecorps_dict_import` (`pos IN ('v','v.fam')`) per `headword ILIKE` durchsucht, dann **jeden Treffer manuell gegen die konkrete `darija`-Zeile auf Wurzel-Ebene geprüft** (nicht automatisch übernommen — Homograph-Gefahr, s.u.). Ergebnis: **27 von 82 verbleibenden Zeilen** bestätigt (`external_confirmed=true, external_confirmed_source='peacecorps'`):
`n7adhar`/`y7addhar`(vorbereiten), `naghsil`/`taghsel`(waschen), `nathmit`/`nathamt`(organisieren), `wiqfit`/`wqift`(aufstehen), `tnathaf`(putzen), `ynaqqiz`(springen), `yitba3 (active)`(drucken), `y7aqeq`(verwirklichen), `yiknus`(fegen), `yu3ad`(versprechen), `yirbut`(binden), `ytaffi`(Licht ausmachen), `ytoll`(auftauchen/reinschauen), `yrakkedh`(schaumig schlagen), `yitqla`(frittiert werden), `ytrena`(trainieren), `yijmed`(gefrieren), `y7jem (derja)`(rasieren), `yaqdhi`(einkaufen), `ybra`(gesund werden), `ydazz`(stoßen/schieben), `yfadhlik`(scherzen), `yduq`(kosten/probieren).

**Wichtige Funde nur über die `senses`-JSONB, nicht über `forms_chatalpha[1]`:** Peace Corps listet bei vielen Einträgen **synonyme Zweit-/Drittformen als eigene `forms`-Arrays innerhalb von `senses`**, nicht im Haupt-`forms_chatalpha`. Diese wurden durch Volltext-Durchsicht der zurückgegebenen `senses`-Spalte gefunden, nicht durch Array-Matching:
- `ytrena` ↔ TRAIN-Eintrag, Hauptform `3allim`, aber Synonym-Note "3 v.(itra:na:)(tra:na:)" listet `tra:na:` — exakter Treffer, wäre bei reinem `forms_chatalpha[1]`-Match verpasst worden.
- `yijmed` ↔ FREEZE-Eintrag, Hauptform `ithlij/thlij` (andere Wurzel, ث-ل-ج "Schnee"), aber Synonym-Note "synonym paradigm (ijmid/jmid)" — exakte Wurzel-Übereinstimmung ج-م-د.
- `ydazz` ↔ PUSH-Eintrag, Hauptform `inzil 3la` (Knopf drücken), Synonym-Note "2 v.(diz)/(daz)" — `dazz` ist die geminierte Form-II-Variante von `daz`.
- `ytaffi` ↔ "Turn off"-Eintrag, Hauptform `sakkir`, Synonym-Note "or v.(aTfi:)(Tfa:)" — Wurzel ط-ف-ي exakt.

**Bewusst NICHT verlinkt trotz oberflächlicher Ähnlichkeit (Homograph-/Wurzel-Fallen):**
- `yitba3 (passive)`("wird verkauft", id3281) — **gleiche Schreibung** wie `yitba3 (active)`("druckt", id2213, ↔ Wurzel ط-ب-ع PRINT), aber andere Wurzel (ب-ي-ع "verkaufen"). Reiner Schreibungs-Zufall, kein Peace-Corps-Treffer für die Verkaufs-Bedeutung gefunden — **nicht verlinkt**.
- `ya3jen`("kneten") vs. Peace-Corps KNEAD-Eintrag `u3ruk/3rak` — andere Wurzel (ع-ر-ك statt ع-ج-ن, offenbar ein regionales Synonym im Peace-Corps-Korpus) — **nicht verlinkt**.
- `yiz7af`("kriechen", Wurzel ز-ح-ف) vs. Peace-Corps CRAWL-Eintrag `a7bi/7ba` (Wurzel ح-ب-و, ein anderes Wort für "krabbeln") — gleiche Bedeutung, andere Wurzel — **nicht verlinkt**.
- `yitza3ab`/`tza3ab`("nerven/belästigen") vs. Peace-Corps ANNOY/BOTHER-Eintrag `qallaq` (Wurzel ق-ل-ق) — andere Wurzel — **nicht verlinkt**.
- `y3addi 3la`/`3adda 3la`("untersuchen") vs. Peace-Corps EXAMINE-Eintrag `qallib` (Wurzel ق-ل-ب) — andere Wurzel — **nicht verlinkt**.
- `y7alliq`/`yaslet`/`yqarr3` (weitere Rasieren-Varianten) vs. SHAVE-Eintrag `7ajjim` — nur `y7jem (derja)` matcht die Wurzel ح-ج-م exakt, die anderen drei sind andere Wurzeln/Bedeutungsnuancen — **nicht verlinkt**.
- `ysuq`/`seqit`/`soqt`("Auto fahren") — einzig gefundener DRIVE-Eintrag war "Drive at (to)" = `uqsid/qsad` ("worauf hinauswollen", andere Bedeutung) — kein echter Treffer, **nicht verlinkt**.
- `dawshit`/`dawesht`("duschen"), `nghib`("fehlen"), `yboss`/`yifsa`(furzen), `yi3wi`(heulen), `yinahhiq`(iahen/Esel), `nirmi`(werfen), `toftor`/`fatrou`(frühstücken), `y7alliq`(kreisen/Vogel), `yb3ad`(weggehen) — zu den durchsuchten Glossen kein `pos IN ('v','v.fam')`-Eintrag in `peacecorps_dict_import` gefunden — genuine Lücke in dieser Quelle.
- Mehrwort-Beispielsätze (`taqra 7atta`, `tel3ab volley`, `trawa7 li eddar`, `t3awin omha`, `titmasha fil parc`, `t7iz kalbha`, `titfarraj fil talfza`, `tel3ab jeux vidéos`, `imshiou aghslou ydikom`) — laut Phrasen-Ausschluss-Regel (s.o.) grundsätzlich nicht für Wörterbuch-Einzelabgleich geeignet — **nicht geprüft**.

**Stand danach: 27/82 zusätzlich bestätigt, 55 Zeilen bleiben unbestätigt** (echte Lücken in Ninja/TUNICO/Peace-Corps oder Mehrwort-Phrasen ohne Lemma-Entsprechung).

### Konjugationstabelle unabhängig von `tunico_verb_id` (seit 2026-09-06)

Die 🔠-Tabelle im Trainer hängt technisch nur an `vocabulary.conjugation` (JSONB), nicht an `tunico_verb_id`/`external_confirmed_source` — sie kann also auch für reine Peace-Corps- (oder gar unbestätigte) Verben gebaut werden, nicht nur für TUNICO-Korpus-Verben. Für 26 der 27 gerade per Peace-Corps bestätigten Zeilen wurde `conjugation` befüllt, **auf User-Wunsch nur mit sicher ableitbaren Zellen** ("nur sichere Formen schreiben, macht nichts wenn nicht die ganze Tabelle gefüllt hat") — keine neuen Zeilen angelegt, kein `conj_rotate` gesetzt, nur die bestehende Zeile um `conjugation` ergänzt:

- **Präsens-Singular (1sg/2sg/3sg_m/3sg_f) ist bei allen regelmäßigen Verben sicher per reinem Präfix-Tausch (n-/t-/y-/t-) auf demselben Stamm ableitbar** — bestätigt anhand mehrerer bereits bestehender vollständiger `conjugation`-Beispiele (`yoshrob`, `ytayyeb`, `yna77i`, `yelbis`: alle 4 Singular-Personen teilen exakt denselben Stamm, nur der Präfix wechselt). Präsens-Plural dagegen braucht oft eine interne Vokal-Synkope vor `-u` (`oshrob→oshrbu`, `tayyeb→tayybu`), die nicht mechanisch sicher ist — **nur ergänzt, wenn der Stamm bereits auf einen Vokal endet** (`taffi→taffiu`, `aqdhi→aqdhiu`, kein Synkope-Risiko), sonst weggelassen.
- **Vergangenheit hat zwei Stämme:** einen "leichten" Stamm für 1./2. Person + Plural (`nathamt`→auch `nathamna`, `nathamtu`, und 3sg_m durch reines Abstreifen des `-t`-Suffixes: `natham`) und einen "schweren"/verschobenen Stamm für 3sg_f/3pl (`nathmit`→auch `nathmu`). Bei `nathmit`/`nathamt` (organisieren) und `wiqfit`/`wqift` (aufstehen vom Platz) waren **beide Stämme bereits aus den 2 bestehenden Zeilen bekannt** (nicht geraten), dadurch ließ sich das **volle 7-Zellen-Paradigma sicher rekonstruieren** — auf beide zugehörigen Zeilen identisch angewendet (wie beim bestehenden `yakol`/`nekol`-Muster: ein Verb, mehrere Zeilen, dieselbe volle `conjugation`).
- **Bewusst weggelassen:** Imperativ (Regel "nur wenn nicht semantisch seltsam" hätte hier für jedes Verb einzeln geprüft werden müssen — zu viel Ermessensspielraum für "sichere Formen"), Präsens-Plural bei konsonantisch endenden Stämmen (Synkope-Vokal unbekannt), und `yitqla`("wird frittiert", Passiv/unpersönlich) komplett — kein natürliches Personen-Paradigma, daher **keine `conjugation`** für diese Zeile.
- Bei `n7adhar`(id1274)/`y7addhar`(id2218) und `naghsil`(id1275)/`taghsel`(id1830) — **zwei bereits bestehende, aber unterschiedlich transliterierte Zeilen für dasselbe Verb** (`7adhar` vs. `7addhar`, `aghsil` vs. `aghsel`) — wurde **bewusst nicht vereinheitlicht**, sondern jede Zeile bekam ihr eigenes, in sich konsistentes Paradigma auf Basis des eigenen Stamms, um keine der beiden bestehenden Schreibweisen zu überschreiben/für falsch zu erklären (außerhalb des Auftragsumfangs).

### ⚠️ Systemischer Blind Spot in `tunico_import`: 535 Zeilen mit `lemma_chatalpha=NULL` (gefunden 2026-09-06)

User-Frage "Gibt es die Wochentage wirklich in keiner der drei Quellen?" (zu `itnin`/Montag id274, `el-irb3a`/Mittwoch id518, beide unbestätigt) deckte auf: **beide waren in Wirklichkeit in TUNICO vorhanden**, nur unsichtbar für jede bisherige Exact-Match-Suche, weil `lemma_chatalpha` bei diesen Einträgen `NULL` ist — das eigentliche Wort steckt nur in `xml_id` (`nhaar_l_itniin_001`, `nhaar_l_irbca_001`) und in `senses` (de/en/fr-Glossen), nicht in den durchsuchten Spalten `lemma_chatalpha`/`variants_chatalpha`/`inflected`. Nachprüfung: **`SELECT count(*) FROM tunico_import WHERE lemma_chatalpha IS NULL` → 535 Zeilen, davon 534 mit echtem `senses`-Inhalt.**

**Das ist eine bislang unbekannte, projektweite Lücke der `external_confirmed_source='tunico'`-Methodik** (Regel in „`external_confirmed`/`external_confirmed_source`" oben) — jede der bisherigen Prüfungen gegen `lemma_chatalpha`/`variants_chatalpha`/`inflected` hat diese 535 Einträge grundsätzlich nie gesehen, nicht nur bei den Wochentagen. Betrifft NICHT die separate `tunico_corpus_verbs`-Tabelle (die 300-Verben-Korpus-Verlinkung, dort ist die "vollständig ausgeschöpft"-Aussage von oben unberührt) — sondern die allgemeine `tunico_import`-Wörterbuchtabelle, die für `external_confirmed='tunico'`-Bestätigungen bei Nicht-Verben (Nomen, Wochentage etc.) herangezogen wird.

**Zusätzlich bei Wochentagen: Peace Corps hatte `MONDAY`→`"nhar lithnin"`/`WEDNESDAY`→`"nhar lirb3a"` (mit `"nhar"`-Präfix), während der Bestand nur `itnin`/`el-irb3a` (ohne Präfix) speichert — ein reiner Exact-Match hätte auch das nie gefunden, nur ein Präfix-toleranter Abgleich.** Beide Zeilen jetzt bestätigt (`external_confirmed=true, external_confirmed_source='tunico'`).

**Noch nicht gemacht: ein systematischer Abgleich der 534 `senses`-befüllten `tunico_import`-Zeilen gegen den gesamten unbestätigten gruppen (nicht nur Verben) — potenziell weitere false-negative "Lücken" wie bei den Wochentagen.** Aufwand vergleichbar mit dem Peace-Corps-Englisch-Gloss-Durchgang oben (pro Zeile `senses`-Text lesen, gegen Bestand matchen), aber mit dem Zusatzschritt "Wort aus `xml_id` extrahieren, da `lemma_chatalpha` fehlt".

**→ Erledigt (2026-09-06, User-Auftrag "ja").** `xml_id` (Unterstriche → Leerzeichen, `_NNN`-Suffix entfernt) approximiert die chatalpha-Schreibung recht gut, da TUNICOs `xml_id`-Konvention dieselben Digraphen wie unser Chat-Alphabet nutzt (`zh`=ج, `x`=خ, `c`=ع, `sh`=ش usw.). Vollständiger `root_skel()`-Substring-Abgleich: für **alle 1248** `external_confirmed=false`-Zeilen des GESAMTEN Bestands (nicht nur Verben) geprüft, ob `root_skel(darija)` als Substring in `root_skel(xml_id_clean)` irgendeiner der 534 Zeilen vorkommt.

**Ergebnis: ~150 Rohtreffer, davon nach manueller Prüfung 28 echte Bestätigungen** (`external_confirmed_source='tunico'`): `ma safia`(Mineralwasser), `nos rtal`(halbes Pfund), `slata meshwiya`/`slata tounssia`(gegrillter/tunesischer Salat), `tounsiya`(tunesisch f.), `min houni`(von hier), `min ghir mziyya`(gern geschehen), `billehi`(bitte), `allah ybarek`, `kol youm`/`lyoum`-NEIN(lyoum war Fehltreffer, s.u.), `srewil`(Hosen), `lwin`(wohin), `liltek zina`(Schöne Nacht), `bismillah`, `ftouri`(mein Frühstück), `mazel`/`mazelou`(noch/noch immer), `lebodda`(unbedingt nötig), `el-kiren`(Stadtbus), `qlam rsas`(Bleistift), `moush normal`, `litaw`(bis jetzt), `warqit il-ma`(Wasserrechnung), `kart postal`(Postkarte), `qahwa shtar`(Milchkaffee), `ma sabbala`(Leitungswasser), `b-zarba`(schnell), `qbal il-waqt`(frühzeitig). (`kol youm`(jeden Tag) bestätigt; das ähnlich klingende `lyoum`(heute) dagegen NICHT — s. Fehltreffer-Absatz unten, andere Bedeutung.)

**Der Rest der Rohtreffer (~120) war Substring-Zufallsrauschen, kein echter Wortbezug** — v.a. bei kurzen Wurzeln (3-4 Konsonanten): `besh`/`baash` matchten in fast jeder langen Phrase, `twel`(lang, Pl.) in `twiil`(geradeaus, anderes Derivat), `aqrab`(näher) in `taqriib`(ungefähr), `mayyita`(tot f.) in `miiyat alf`(hunderttausend), `lyoum`(heute) in `kull yuum`(jeden Tag, anderes Wort trotz Wurzelverwandtschaft) — Buchstaben-Überlappung ohne Wurzel-/Bedeutungsbezug. **Neue Regel: Substring-Match auf `root_skel()` ist bei Wurzellänge <5 Zeichen zu unzuverlässig für automatisches Bestätigen — jeder Treffer braucht manuelle Prüfung auf a) echte Wurzel-Identität (nicht nur Zeichenkette) und b) Bedeutungs-Übereinstimmung.**

**Bewusst NICHT bestätigt trotz plausibler Wurzel-Verwandtschaft:** Fälle, wo der Bestand eine **flektierte Form (Plural/Possessiv)** ist und der TUNICO-Fund nur die **Grundform eingebettet in einem unabhängigen Idiom** enthält — z.B. `bnet`/`bneti`(Töchter/meine Tochter) vs. `bint 3ayla`("braves Mädchen aus gutem Haus", enthält nur die Singularform `bint`), `awled`(Kinder) vs. `wild 3ayla`("braver Junge", nur Singular `wild`), `kleb`(Hunde) vs. `kalb bhar`("Hai", nur Singular `kalb`) — die flektierte Form selbst ist in keinem der Funde direkt belegt, nur die Wurzel. Ebenso `nhari`(mein Tag) gegen diverse `nhaar`-Komposita (Wochentage, "Mittag" u.a.) — keiner der Funde bedeutet tatsächlich "mein Tag".

**Dieser Durchgang ist vollständig** (alle 1248×534 Kombinationen per SQL geprüft, nicht nur eine Stichprobe) — für den aktuellen Bestand bleibt kein `tunico_import`-Treffer mehr unentdeckt, der über einen `root_skel()`-Substring-Abgleich auffindbar wäre. Echte Lücken (kein Treffer) oder zu unsichere Fälle (s.o.) bleiben unbestätigt.
