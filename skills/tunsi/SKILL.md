...
- **Systematischer Check danach: breite `ILIKE`/Regex-Suche über ~90 weitere geratene Lemmata (kneten, auspressen, schälen, kahlrasieren, servieren, trainieren, Ei aufschlagen, Tor schießen, iahen, kriechen, wiegen, Fasten brechen, u.v.m.) ergab NULL Treffer** — diese Verben stehen schlicht nicht in den 300 häufigsten `tunico_corpus_verbs`. **Endgültiger Stand 2026-09-06: 114/300 `tunico_verb_id`-verlinkt.** Von den verbleibenden ~244 unlinked `Verben-Konjugation`/`Vergangenheit`/`Verben`-Zeilen sind 162 bereits über `external_confirmed`(Ninja/Peace Corps) bestätigt (nur eben nicht ans Korpus-Konjugationsmodell angebunden, weil das jeweilige Verb dort fehlt) — für diese ist nichts weiter zu tun außer optional eine eigene `conjugation`-Tabelle von Hand zu bauen (ohne `tunico_verb_id`-Link, dann ohne Korpus-Absicherung der Vokale). Die übrigen 82 sind weder verlinkt noch extern bestätigt — Kandidaten für einen künftigen gezielten Ninja/Peace-Corps-Check, nicht für weitere TUNICO-Korpus-Suche.
