...
1. **Offline-Dump zuerst:** gegen `derja_ninja_entries` prüfen (siehe Datenquellen unten) — schnell, aber ein Snapshot (2026-08-17), kann bei mehrteiligen Begriffen unvollständig sein.
2. **Live-Check, wenn der Dump nichts liefert:** `GET https://derja.ninja/search?search=<begriff>&script=<english|transliterated|arabic>` (serverseitig gerendert, curl reicht, kein JS nötig). `script=english` für deutsche/englische Begriffe (vorher übersetzen), `script=arabic` für arabische Schreibung, `script=transliterated` für lateinische Umschrift (Umwandlungstabelle siehe unten). Kein Treffer → sauberer Text "No results".
...
**Nutzen für den Ninja-Check-Workflow:** bei geflaggten/neuen Vokabeln ohne Treffer in `derja_ninja_entries` oder `tunico_import` zusätzlich `peacecorps_dict_import` durchsuchen (Englisch-Übersetzung als Suchschlüssel) — dritte unabhängige Quelle, bevor eine Vokabel als "kein Treffer, keine externe Bestätigung" gilt.
