...
- **Noch offen (2026-09-06 Abend): ~269 `topic IN ('Verben-Konjugation','Vergangenheit','Verben')`-Zeilen mit `tunico_verb_id IS NULL`** — enthält weitere kurzwurzelige Grundverben (u.a. Passiv-/Zustandsverben wie `fsed`/verderben, `bred`/kalt werden, `mradh`/krank werden, die schon an anderer Stelle korrekt verlinkt sind, aber auch noch unentdeckte). Nächster Schritt: dieselbe Lemma-Ratemethode auf den Rest der 360er-Liste anwenden (deutsche Glossen gruppiert durchgehen, Kandidaten-Lemmata gegen `tunico_corpus_verbs.lemma_chatalpha` per `ILIKE` prüfen), dabei bewusst zwischen "echtes Korpus-Verb, nur falsch gebucketed" und "existiert im Korpus schlicht nicht" (dann ggf. nur `external_confirmed` über Ninja/Peace Corps setzen, kein `tunico_verb_id`) unterscheiden.
