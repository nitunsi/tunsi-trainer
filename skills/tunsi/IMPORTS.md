...
**TUNICOs Lemma ist immer die Grundform — für Plurale, Duale und Flexionsformen `inflected` benutzen.** Die Spalte trägt Einträge der Form `{ana, orig, chatalpha}` mit Analysetag (`#n_pl`, `#n_dual`, `#n_unit_dual`, `#v_pres_sg_p3`). Ohne sie zieht ein Skelettabgleich den Plural systematisch auf den Singular: `fnejin` „Tassen" auf `finjan` „Tasse", `mgharif` „Löffel (Pl.)" auf `mgharfa`, `amriken` „Amerikaner (Pl.)" auf `amrikani` „amerikanisch". Unser Bestand ist voll von Pluralen, das ist also kein Randfall. Ebenso: trägt die `darija` einen **Artikel** (`el-`, `esh-`, `ez-`), muss er vor der Suche abgetrennt und danach wieder vorangestellt werden — sonst zählt das Skelett ihn mit und trifft nie ein Lemma (`el-7imaya`, `esh-shahada`, `el-qahwa`).
...
**Regel: arabic_script aus Derja Ninja bevorzugen**, auch bei kleinen Abweichungen — **aber erst, nachdem feststeht, dass es dasselbe WORT ist.** Buchstabengleichheit ist kein Wortnachweis: beim vollständigen Durchgang durch `vokalisierung_kandidaten` am 2026-09-15 waren **10 von 21 buchstabengleichen, eindeutig vokalisierten Treffern Homograph-Zufälle** — die Eigennamen نَجِيب (Najib) für `njib` „ich bringe" und سَمَر (Samar) für `smar` „er wurde braun", der osmanische Titel بَايْ (Bey) für das Lehnwort „bye", die Maß-II-Form تْكَوِّن „geformt werden" für تْكُون „sein", der Diminutiv صْغَيَّر für صغير, ein Verbalnomen تَفَاهُم statt des Verbs, das Verb كَتَبْ „to write" für den Nomen-Plural „Bücher". Pflicht vor der Übernahme: Ninjas `english` gegen unser `german` halten und Wortart/Verb-Maß prüfen (Lautlehre-Regel 5). Bei Zweifel ausschließen und mit `internal_note ~ 'ninja-vokalisierung verworfen'` festhalten. `darija` danach an die neue Form anpassen. `german`-Herkunftshinweis nur bei echtem Mehrwert ergänzen ((MSA)/(Darija)/(frz.)). `lesson_id` prüfen, bei Unklarheit Rückfrage. **Nicht automatisch ändern:** bereits korrekt vokalisierte Einträge mit nur orthographischer Variante bei Ninja; abweichende Bedeutung → als Auffälligkeit markieren, Nutzer entscheiden lassen; `darija` selbst wird nie aus Ninja übernommen (andere Konvention).
